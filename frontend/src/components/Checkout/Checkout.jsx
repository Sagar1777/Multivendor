...
const Checkout = () => {
  ...
  // Calculate the total discount based on the cart and the coupon code
  const calculateTotalDiscount = (couponCodeValue) => {
    return cart.reduce((acc, item) => {
      if (item.shopId === couponCodeValue.shopId) {
        return acc + (item.qty * item.discountPrice * couponCodeValue.value) / 100;
      }
      return acc;
    }, 0);
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (couponCodeValue !== null) {
        const isCouponValid = cart.some((item) => item.shopId === couponCodeValue.shopId);
        ...
        if (!isCouponValid) {
          ...
          const totalDiscount = calculateTotalDiscount(couponCodeValue);
      ...
      toast.error("Error fetching coupon data. Please try again later.");
    }
  };

  // Calculate the total price with discount applied only if there's a valid coupon code
  ...
  return (
    <div className="w-full bg-[#fff] rounded-md p-5 pb-8">
      ...
      <div className="flex justify-between border-b pb-3">
        ...
        <h5 className="text-[18px] font-[600]">Rs
